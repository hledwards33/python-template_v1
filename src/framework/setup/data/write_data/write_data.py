...
from framework.setup.data.data_checks.check_output_data import DataCheckFactory, IDataCheck, DataCheckContext
...
class WriteDataBuilder(IWriteDataBuilder):
    ...
    def set_data_checker(self) -> IDataCheck:
        check_context = DataCheckContext(self.context.data, self.context.model_type)
        return DataCheckFactory.get_data_checker(check_context)
...
class WriteDataDirector:
    ...
    def write_data(self):
        ...
        self.builder.file_saver.save_file(formatted_schema)
        return errors
